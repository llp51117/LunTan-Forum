...
<script>
    ...
    const elements = {
        ...
        avatarFile: document.getElementById('avatarFile'), // 对应页面中隐藏的<input type="file">
        userAvatar: document.getElementById('userAvatar') // 头像显示容器
    };
    ...
    function initAvatarUpload() {
        ...
        // 2. 选择文件后执行上传逻辑
        elements.avatarFile?.addEventListener('change', handleAvatarChange);
    }
    ...
    function handleAvatarChange(e) {
        ...
        console.log('选中的文件：', {
            文件名: file.name,
            大小: file.size + '字节',
            类型: file.type // 应该是image/jpg、image/png等
        });
        // 调用上传接口（这里单独拆分成函数，逻辑更清晰）
    ...
    function uploadAvatarToServer(file) {
        ...
        // 改成和后端一致的路径 + 项目名（必填！）
        fetch('/luntan/AvatarUploadServlet', {
            method: 'POST',
            body: formData
        })
            .then(res => res.json())
            .then(data => {
                if (data.success) {
                    alert('头像上传成功！');
                    elements.userAvatar.style.backgroundImage = `url(${data.avatarUrl})`;
